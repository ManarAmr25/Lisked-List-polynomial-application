...
class DoublyLinkedListTest {
	...
	void test0() {
		...
		assertThrows(RuntimeException.class, ()->{L.add(null);});		 // null will not be added - size will not increase
		assertEquals(7,L.size());
		
		assertEquals(false, L.contains(2000));
		assertEquals(false, L.contains(null));
		assertEquals(true, L.contains(x));
		assertEquals(true, L.contains('c'));
		
		
		assertEquals(null,L.get(-5));
		//assertThrows(RuntimeException.class, ()->{L.get(-5);});
		assertEquals("Hello world!",L.get(0));
		assertEquals(x , L.get(1));
		assertEquals(523 , L.get(2));
		//assertEquals('c' , L.get(3));
		//assertEquals(52369.1023 , L.get(4));
		//assertEquals(20.5f , L.get(5));
		assertEquals(true , L.get(6));
		assertEquals(null,L.get(7));
		//assertThrows(RuntimeException.class, ()->{L.get(7);}); // there is no object at index 7
		//assertEquals(null , L.get(20));
		...
		//assertEquals(523 , L.get(3));
		//assertEquals('c' , L.get(4));
		//assertEquals(52369.1023 , L.get(5));
